...
var _ = Describe("VM Network Controller", func() {
	...
	It("sync handles multiple NAD reference updates", func() {
		...
		expectedNets := []v1.Network{
			...
			*libvmi.MultusNetwork(secondaryNetName1, updatedNADName1),
			*libvmi.MultusNetwork(secondaryNetName2, updatedNADName2),
			*libvmi.MultusNetwork(secondaryNetName3, nadName3),
		}
		Expect(updatedVM.Spec.Template.Spec.Domain.Devices.Interfaces).To(Equal(expectedIfaces))
		Expect(updatedVM.Spec.Template.Spec.Networks).To(Equal(expectedNets))
